...
async fn fresh_pool() -> PgPool {
    ...
    reset_ingress_tables(&pool).await;
    pool
}

async fn reset_ingress_tables(pool: &PgPool) {
    sqlx::query(
        r#"
        TRUNCATE outbound_messages, expenses, conversation_states, provider_identities,
                 inbound_events, accounts RESTART IDENTITY CASCADE
        "#,
    )
    .execute(pool)
    .await
    ...
    sqlx::query(
        "UPDATE ingress_control SET mode = 'webhook', mode_generation = 1, updated_at = NOW() WHERE id = 1",
    )
    .execute(pool)
    .await
    .expect("reset ingress mode");
}
...
async fn manual_draft_pending_and_confirm_flow() {
        ...
        .process(base_request("evt-manual", "sender-manual"), move |_ctx| {
            Ok(DecisionOutput {
                effects: vec![IngressEffect::CreateManualExpenseAwaitingConfirmation {
                    ...
                    occurred_at: Utc::now(),
                    pending_action_type: "confirm_expense".to_string(),
                }],
                reply: Some(ReplyIntent {
    ...
    .await
    .expect("pending");
    assert_eq!(pending_type, "confirm_expense");
...
async fn pending_version_conflict_rolls_back_entire_transaction() {
    ...
    assert_eq!(pending_type.as_deref(), Some("confirm_expense"));
}
